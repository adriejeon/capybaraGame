...
def convert_png_to_webp(png_path: Path) -> bool:
    ...
    try:
        ...
        print(f"✗ 변환 실패: {png_path.name} - {str(e)}")
        return False


def find_and_convert_pngs(directory: Path):
    """
    디렉토리를 재귀적으로 순회하며 PNG 파일을 찾아 WebP로 변환
    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            ...
            if file_path.suffix.lower() == '.png':
                ...
                if webp_path.exists():
                    print(f"⚠ 건너뜀 (이미 WEBP 존재): {file_path.name}")
                    continue
                
                # WebP로 변환
                if convert_png_to_webp(file_path):
                    # 변환 성공 시 원본 PNG 파일 삭제
                    try:
                        file_path.unlink()
                        converted_count += 1
                    except Exception as e:
...
def update_dart_files(directory: Path):
    """
    Dart 파일 내의 PNG 이미지 경로 확장자를 .webp로 변경
...
def main():
    ...
    find_and_convert_pngs(ASSETS_DIR)
