...
class CollectionItem {
  ...
  CollectionItem copyWith({bool? isUnlocked, bool? isNew, int? storyId, GameDifficulty? difficulty}) {
    return CollectionItem(
      id: id,
...
class CollectionManager {
  ...
  List<Story> _createDefaultStories() {
    return [
      ...
      Story(
        ...
        descriptionEn: 'A story about a small capybara taking its first steps into the world.',
  ...
  Future<void> _migrateCollectionSize() async {
    ...
    final int expectedCount = defaultCollection.length; // 80개 (Level1: 20, Level2: 20, Level3: 20, Level4: 10, Level5: 10)
    
    print('[마이그레이션] 현재 컬렉션 크기: ${_collection.length}, 예상 크기: $expectedCount');
    print('[마이그레이션] 현재 컬렉션 ID 범위: ${_collection.isEmpty ? "비어있음" : "${_collection.first.id} ~ ${_collection.last.id}"}');
    
    // **중요**: 난이도별 슬롯 개수 검증 및 수정
    final level1Count = _collection.where((item) => item.difficulty == GameDifficulty.level1).length;
    final level2Count = _collection.where((item) => item.difficulty == GameDifficulty.level2).length;
    final level3Count = _collection.where((item) => item.difficulty == GameDifficulty.level3).length;
    final level4Count = _collection.where((item) => item.difficulty == GameDifficulty.level4).length;
    final level5Count = _collection.where((item) => item.difficulty == GameDifficulty.level5).length;
    
    print('[마이그레이션] 난이도별 슬롯: L1=$level1Count(20), L2=$level2Count(20), L3=$level3Count(20), L4=$level4Count(10), L5=$level5Count(10)');
    
    // **중요**: ID 범위 기반 난이도 검증 및 수정
    // 슬롯 ID 1-20번은 반드시 level1이어야 함
    final wrongLevel1Slots = _collection
        .where((item) => item.id >= 1 && item.id <= 20 && item.difficulty != GameDifficulty.level1)
        .toList();
    if (wrongLevel1Slots.isNotEmpty) {
      ...
      for (var wrong in wrongLevel1Slots) {
        print('  - 슬롯 ID: ${wrong.id}, 잘못된 난이도: ${wrong.difficulty.name} -> level1로 수정');
        final wrongIndex = _collection.indexWhere((item) => item.id == wrong.id);
        if (wrongIndex != -1) {
          _collection[wrongIndex] = wrong.copyWith(difficulty: GameDifficulty.level1);
          needsSave = true;
        }
      }
    }
    ...
    final wrongLevel2Slots = _collection
        .where((item) => item.id >= 21 && item.id <= 40 && item.difficulty != GameDifficulty.level2)
        .toList();
    if (wrongLevel2Slots.isNotEmpty) {
      print('[마이그레이션] ID 21-40번 범위에 잘못된 난이도 슬롯 발견: ${wrongLevel2Slots.length}개');
      for (var wrong in wrongLevel2Slots) {
        print('  - 슬롯 ID: ${wrong.id}, 잘못된 난이도: ${wrong.difficulty.name} -> level2로 수정');
        final wrongIndex = _collection.indexWhere((item) => item.id == wrong.id);
        if (wrongIndex != -1) {
          _collection[wrongIndex] = wrong.copyWith(difficulty: GameDifficulty.level2);
          needsSave = true;
        }
      }
    }
    ...
    final wrongLevel3Slots = _collection
        .where((item) => item.id >= 41 && item.id <= 60 && item.difficulty != GameDifficulty.level3)
        .toList();
    if (wrongLevel3Slots.isNotEmpty) {
      print('[마이그레이션] ID 41-60번 범위에 잘못된 난이도 슬롯 발견: ${wrongLevel3Slots.length}개');
      for (var wrong in wrongLevel3Slots) {
        print('  - 슬롯 ID: ${wrong.id}, 잘못된 난이도: ${wrong.difficulty.name} -> level3로 수정');
        final wrongIndex = _collection.indexWhere((item) => item.id == wrong.id);
        if (wrongIndex != -1) {
          _collection[wrongIndex] = wrong.copyWith(difficulty: GameDifficulty.level3);
          needsSave = true;
        }
      }
    }
    ...
    final wrongLevel4Slots = _collection
        .where((item) => item.id >= 61 && item.id <= 70 && item.difficulty != GameDifficulty.level4)
        .toList();
    if (wrongLevel4Slots.isNotEmpty) {
      print('[마이그레이션] ID 61-70번 범위에 잘못된 난이도 슬롯 발견: ${wrongLevel4Slots.length}개');
      for (var wrong in wrongLevel4Slots) {
        print('  - 슬롯 ID: ${wrong.id}, 잘못된 난이도: ${wrong.difficulty.name} -> level4로 수정');
        final wrongIndex = _collection.indexWhere((item) => item.id == wrong.id);
        if (wrongIndex != -1) {
          _collection[wrongIndex] = wrong.copyWith(difficulty: GameDifficulty.level4);
          needsSave = true;
        }
      }
    }
    ...
    final wrongLevel5Slots = _collection
        .where((item) => item.id >= 71 && item.id <= 80 && item.difficulty != GameDifficulty.level5)
        .toList();
    if (wrongLevel5Slots.isNotEmpty) {
      print('[마이그레이션] ID 71-80번 범위에 잘못된 난이도 슬롯 발견: ${wrongLevel5Slots.length}개');
      for (var wrong in wrongLevel5Slots) {
        print('  - 슬롯 ID: ${wrong.id}, 잘못된 난이도: ${wrong.difficulty.name} -> level5로 수정');
        final wrongIndex = _collection.indexWhere((item) => item.id == wrong.id);
        if (wrongIndex != -1) {
          _collection[wrongIndex] = wrong.copyWith(difficulty: GameDifficulty.level5);
          needsSave = true;
        }
      }
    }
    ...
    if (level1Count != 20) {
      ...
      _collection.removeWhere((item) => item.difficulty == GameDifficulty.level1);
      
      for (int i = 1; i <= 20; i++) {
        if (existingItems.containsKey(i) && existingItems[i]!.difficulty == GameDifficulty.level1) {
    ...
    if (level2Count != 20) {
      ...
      final existingLevel2Items = _collection
          ...
          .toList();
      final story2Items = existingLevel2Items.where((item) => item.storyId == 2).toList();
      final story8Items = existingLevel2Items.where((item) => item.storyId == 8).toList();
      
      // 기존 Level2 아이템 제거
      _collection.removeWhere((item) => item.difficulty == GameDifficulty.level2);
    ...
    if (level3Count != 20) {
      ...
      final existingLevel3Items = _collection
          ...
          .toList();
      final story3Items = existingLevel3Items.where((item) => item.storyId == 3).toList();
      final story9Items = existingLevel3Items.where((item) => item.storyId == 9).toList();
      
      // 기존 Level3 아이템 제거
      _collection.removeWhere((item) => item.difficulty == GameDifficulty.level3);
    ...
    if (level4Count != 10) {
      ...
      _collection.removeWhere((item) => item.difficulty == GameDifficulty.level4);
      
      for (int i = 61; i <= 70; i++) {
        if (existingItems.containsKey(i) && existingItems[i]!.difficulty == GameDifficulty.level4) {
          final existingItem = existingItems[i]!;
    ...
    if (level5Count != 10) {
      ...
      _collection.removeWhere((item) => item.difficulty == GameDifficulty.level5);
      
      for (int i = 71; i <= 80; i++) {
        if (existingItems.containsKey(i) && existingItems[i]!.difficulty == GameDifficulty.level5) {
          final existingItem = existingItems[i]!;
    ...
    for (var defaultItem in defaultCollection) {
      if (!_collection.any((item) => item.id == defaultItem.id)) {
        ...
        needsSave = true;
        addedCount++;
        print('[마이그레이션] 슬롯 ${defaultItem.id} 추가 (difficulty: ${defaultItem.difficulty.name}, storyId: ${defaultItem.storyId})');
      }
    }
    ...
    if (needsSave) {
      await _saveCollection();
      print('[마이그레이션] ${addedCount > 0 ? "$addedCount개 슬롯 추가," : ""} 총 ${_collection.length}개 슬롯');
    } else {
      print('[마이그레이션] 변경사항 없음');
    }
    
    // 신의 경지 단계의 슬롯을 항상 10개로 제한 (71-80만 유지, 81 이상 모두 제거)
    final level5Items = _collection.where((item) => item.difficulty == GameDifficulty.level5).toList();
    final level5ItemsToRemove = level5Items.where((item) => item.id > 80).toList();
    ...
    final finalLevel5Count = _collection.where((item) => item.difficulty == GameDifficulty.level5).length;
    if (finalLevel5Count != 10) {
      // 71-80번 슬롯만 남기고 나머지 모두 제거
      final allLevel5Items = _collection.where((item) => item.difficulty == GameDifficulty.level5).toList();
      final itemsToRemove = allLevel5Items.where((item) => item.id < 71 || item.id > 80).toList();
      ...
      // 71-80번 슬롯이 없으면 추가
      for (int i = 71; i <= 80; i++) {
        if (!_collection.any((item) => item.id == i && item.difficulty == GameDifficulty.level5)) {
          _collection.add(CollectionItem(
            id: i,
            difficulty: GameDifficulty.level5,
            imagePath: _defaultImagePath,
  ...
  Future<void> _validateAndFixStoryDifficulties() async {
    ...
    for (int i = 0; i < _stories.length; i++) {
      ...
      if (story.difficulty != correctDifficulty) {
        print('[수정] 스토리 ID ${story.id}의 난이도 수정: ${story.difficulty.name} -> ${correctDifficulty.name}');
        _stories[i] = Story(
          id: story.id,
  ...
  Future<CollectionResult> addNewCard(GameDifficulty difficulty) async {
    ...
    // 디버그: 현재 컬렉션 상태 출력
    final level1Items = _collection.where((item) => item.difficulty == GameDifficulty.level1).toList();
    print('[디버그] Level1 전체 슬롯 수: ${level1Items.length}');
    print('[디버그] Level1 잠금 해제된 슬롯: ${level1Items.where((item) => item.isUnlocked).length}');
    print('[디버그] Level1 storyId 1 슬롯: ${level1Items.where((item) => item.storyId == 1).length}');
    print('[디버그] Level1 storyId 7 슬롯: ${level1Items.where((item) => item.storyId == 7).length}');
    ...
    print('[디버그] 해당 난이도(${difficulty.name})에서 수집한 카드 수: ${unlockedCardsInDifficulty.length}');
  ...
  Future<CollectionResult> _drawNewCard(
    ...
    // 랜덤으로 새 카드 선택
    final selectedImagePath = availableCards[random.nextInt(availableCards.length)];
    ...
    if (!_isImagePathValidForDifficulty(difficulty, selectedImagePath)) {
      print('[에러] 잘못된 이미지 경로: 난이도 ${difficulty.name}에 $selectedImagePath는 사용할 수 없습니다!');
      // 잘못된 카드 제거하고 다시 시도
      final validCards = availableCards.where((path) => _isImagePathValidForDifficulty(difficulty, path)).toList();
    ...
    final targetStoryId = _getStoryIdForImagePath(difficulty, selectedImagePath);
    ...
    if (lockedCardsInTargetStory.isNotEmpty) {
      ...
      if (wrongDifficultySlots.isNotEmpty) {
        print('[경고] 잘못된 난이도 슬롯 발견: ${wrongDifficultySlots.map((s) => 'ID=${s.id}, 난이도=${s.difficulty.name}').join(', ')}');
      }
    }
    
    print('[디버그] targetStoryId($targetStoryId)의 잠금된 슬롯 개수: ${lockedCardsInTargetStory.length} (난이도: ${difficulty.name})');
    ...
    if (lockedCardsInTargetStory.isEmpty) {
      ...
      if (wrongSlots.isNotEmpty) {
        print('[경고] 잘못된 슬롯 발견 (anyLockedSlots): ${wrongSlots.map((s) => 'ID=${s.id}, 난이도=${s.difficulty.name}').join(', ')}');
        // 잘못된 슬롯 제거
      ...
      print('[디버그] 같은 난이도(${difficulty.name})의 빈 슬롯 개수: ${anyLockedSlots.length} (ID 범위: ${idRange.start}-${idRange.end})');
      ...
      final existingCard = _collection.firstWhere(
        ...
        orElse: () => CollectionItem(
            id: -1, difficulty: difficulty, imagePath: '', isUnlocked: false, storyId: 0),
      );
      ...
      if (selectedSlot.difficulty != difficulty) {
        print('[에러] 슬롯 할당 실패: 슬롯 난이도(${selectedSlot.difficulty.name})와 게임 난이도(${difficulty.name})가 일치하지 않습니다!');
        // 같은 난이도의 다른 슬롯 찾기
        ...
        final correctIndex = _collection.indexWhere((item) => item.id == correctSlot.id);
        ...
        print('[디버그] 슬롯 할당 완료 (수정됨): ID=${correctSlot.id}, 난이도=${difficulty.name}, 이미지=$selectedImagePath, storyId=$targetStoryId');
        ...
        return CollectionResult(
            isNewCard: true,
            card: _collection[correctIndex]);
      }
      
      // **최종 검증**: 선택된 이미지 경로가 슬롯의 난이도와 일치하는지 확인
      if (!_isImagePathValidForDifficulty(selectedSlot.difficulty, selectedImagePath)) {
        print('[에러] 슬롯 할당 실패: 슬롯 난이도(${selectedSlot.difficulty.name})와 이미지 경로($selectedImagePath)가 일치하지 않습니다!');
      ...
      print('[디버그] 슬롯 할당 완료: ID=${selectedSlot.id}, 난이도=${difficulty.name}, 이미지=$selectedImagePath, storyId=$targetStoryId');
      
      // **중요**: 할당 직후 실제 저장된 데이터 확인
      final savedItem = _collection[index];
      print('[디버그] 실제 저장된 데이터: ID=${savedItem.id}, 난이도=${savedItem.difficulty.name}, 이미지=${savedItem.imagePath}, storyId=${savedItem.storyId}, 잠금해제=${savedItem.isUnlocked}');
      
      // **중요**: 할당 후 전체 컬렉션에서 해당 이미지 경로가 몇 개나 있는지 확인
      final duplicateCheck = _collection.where((item) => 
          item.isUnlocked && 
          item.imagePath == selectedImagePath).toList();
      if (duplicateCheck.length > 1) {
        print('[에러] 중복 감지! $selectedImagePath가 ${duplicateCheck.length}개 슬롯에 저장되어 있습니다:');
        for (var dup in duplicateCheck) {
          print('  - 슬롯 ID: ${dup.id}, 난이도: ${dup.difficulty.name}, storyId: ${dup.storyId}');
        }
        // 중복 제거: 가장 최근 것만 남기고 나머지는 잠금
        for (int i = 0; i < duplicateCheck.length - 1; i++) {
          final dupItem = duplicateCheck[i];
          final dupIndex = _collection.indexWhere((item) => item.id == dupItem.id);
      ...
      if (wrongDifficultyInRange.isNotEmpty) {
        print('[에러] ID 범위 ${idRangeCheck.start}-${idRangeCheck.end}에 잘못된 난이도 슬롯 발견:');
        for (var wrong in wrongDifficultyInRange) {
          print('  - 슬롯 ID: ${wrong.id}, 잘못된 난이도: ${wrong.difficulty.name}, 올바른 난이도: ${difficulty.name}');
          // 자동 수정
          final wrongIndex = _collection.indexWhere((item) => item.id == wrong.id);
          if (wrongIndex != -1) {
            _collection[wrongIndex] = _collection[wrongIndex].copyWith(difficulty: difficulty);
            print('[수정] 슬롯 ID ${wrong.id} 난이도 수정: ${wrong.difficulty.name} -> ${difficulty.name}');
          }
        }
      }
    ...
    if (existingCardGlobal.id != -1) {
      ...
      print('[경고] 중복 카드 발견: $selectedImagePath가 이미 슬롯 ID ${existingCardGlobal.id}에 저장되어 있습니다 (난이도: ${existingCardGlobal.difficulty.name})');
    ...
    if (selectedSlot.id < expectedIdRange.start || selectedSlot.id > expectedIdRange.end) {
      print('[에러] 슬롯 ID 범위 오류: 슬롯 ID ${selectedSlot.id}는 난이도 ${difficulty.name}의 범위(${expectedIdRange.start}-${expectedIdRange.end})에 없습니다!');
      // 올바른 범위의 슬롯 찾기
      ...
      print('[디버그] 슬롯 할당 완료 (ID 범위 수정): ID=${correctSlot.id}, 난이도=${difficulty.name}, 이미지=$selectedImagePath, storyId=$targetStoryId');
    ...
    // **중요**: 슬롯의 난이도가 현재 게임 난이도와 일치하는지 확인
    if (selectedSlot.difficulty != difficulty) {
      print('[에러] 슬롯 난이도 불일치: 슬롯 난이도(${selectedSlot.difficulty.name})와 게임 난이도(${difficulty.name})가 일치하지 않습니다!');
      // 같은 난이도의 올바른 슬롯 찾기
      final correctSlots = lockedCardsInTargetStory
          .where((item) => item.difficulty == difficulty)
          .toList();
    ...
    if (existingCard.id != -1) {
      // 이미 존재하는 카드인 경우 (중복 방지)
      print('[경고] 중복 카드 발견: $selectedImagePath가 이미 슬롯 ID ${existingCard.id}에 저장되어 있습니다 (난이도: ${existingCard.difficulty.name})');
      return CollectionResult(isNewCard: false, card: existingCard);
    }
    
    // **추가 검증**: 슬롯 ID가 해당 난이도의 올바른 범위에 있는지 확인
    final slotIdRange = _getExpectedIdRangeForDifficulty(difficulty);
    if (selectedSlot.id < slotIdRange.start || selectedSlot.id > slotIdRange.end) {
      print('[에러] 슬롯 ID 범위 오류: 슬롯 ID ${selectedSlot.id}는 난이도 ${difficulty.name}의 범위(${slotIdRange.start}-${slotIdRange.end})에 없습니다!');
      // 올바른 범위의 슬롯 찾기
      final correctSlots = _collection
          .where((item) => 
              item.difficulty == difficulty && 
      ...
      // **중요**: 할당 후 전체 컬렉션에서 해당 이미지 경로가 몇 개나 있는지 확인
      final duplicateCheck2 = _collection.where((item) => 
          item.isUnlocked && 
          item.imagePath == selectedImagePath).toList();
      if (duplicateCheck2.length > 1) {
        print('[에러] 중복 감지! $selectedImagePath가 ${duplicateCheck2.length}개 슬롯에 저장되어 있습니다:');
  ...
  bool _isImagePathValidForDifficulty(GameDifficulty difficulty, String imagePath) {
    // 이미지 경로에서 접두사 추출
    final match = RegExp(r'([a-z]+)(\d+)\.jpg$').firstMatch(imagePath);
  ...
  int _getStoryIdForImagePath(GameDifficulty difficulty, String imagePath) {
    // 이미지 경로에서 접두사와 번호 추출
    // 예: assets/capybara/collection/easy14.jpg -> easy, 14
    final match = RegExp(r'([a-z]+)(\d+)\.jpg$').firstMatch(imagePath);
    if (match == null) {
    ...
    print('[경고] storyId를 찾을 수 없음: difficulty=$difficulty, imagePath=$imagePath');
    return 1; // 기본값
  }
  ...
  List<CollectionItem> getCardsByStoryId(int storyId) {
    ...
    for (final cardId in story.cardIds) {
      ...
      if (item.difficulty != story.difficulty) {
        print('[수정] getCardsByStoryId: 슬롯 ID $cardId의 난이도 수정: ${item.difficulty.name} -> ${story.difficulty.name}');
        // 실제 컬렉션 데이터 수정
